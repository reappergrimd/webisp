...
$this->mailer->SMTPDebug = 2;                                 // Enable verbose debug output
    $this->mailer->Host = '192.185.112.222';  // Specify main and backup SMTP servers
    $this->mailer->SMTPAuth = true;                               // Enable SMTP authentication
   $this->mailer->Username = 'smtp@pandcict.co.za';                 // SMTP username
    $this->mailer->Password = 'pandc2015';                           // SMTP password
    $this->mailer->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
    $this->mailer->Port = 587;                                    // TCP port to connect to
		$this->mailer->Subject = "Client Connection Request";

		$host = isset($_SERVER['SERVER_NAME'])?$_SERVER['SERVER_NAME']:'localhost';
        $from_email ='forms@'.$host;
   		$this->mailer->setFrom($from_email,'P and C Online Request',false);  
